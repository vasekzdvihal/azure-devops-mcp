...
import { buildWiql, WorkItemsReadService } from '../../../../src/domains/workItems/readService.js';
...
describe('buildWiql', () => {
  it('myActive filters by @me and open states', () => {
    const query = buildWiql({ filter: 'myActive' });
    expect(query).toContain('[System.AssignedTo] = @me');
    expect(query).toContain('NOT IN (\'Closed\',\'Removed\',\'Done\')');
  });
