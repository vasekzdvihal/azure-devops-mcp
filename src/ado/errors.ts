...
function mapStatusCode(shape: SdkErrorShape, detail: string | undefined): AdoError | null {
  ...
  if (shape.statusCode === HTTP_NOT_FOUND) {
    return new AdoNotFoundError(detail);
  }
  if (shape.statusCode === HTTP_CONFLICT) {
    return new AdoConflictError(detail);
  }
  return null;
}

function mapErrorCode(code: string | undefined, detail: string | undefined): AdoError | null {
  switch (code) {
    case 'ECONNREFUSED':
    case 'ETIMEDOUT':
    case 'ENOTFOUND':
...
export function mapSdkError(err: unknown): AdoError {
  ...
  return mapStatusCode(shape, detail) ?? mapErrorCode(shape.code, detail) ?? new AdoUnknownError(detail);
}
