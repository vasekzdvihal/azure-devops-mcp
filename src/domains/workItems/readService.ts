...
function shapeComment(comment: WorkItemComment): WorkItemCommentInfo {
  const createdBy = (comment as { createdBy?: { displayName?: string } }).createdBy?.displayName;
  const createdDate = (comment as { createdDate?: Date }).createdDate;
  return {
...
export class WorkItemsReadService {
  ...
  async get(args: { project: string; workItemId: number }): Promise<WorkItemDetail> {
    ...
    const relations: WorkItemRelationInfo[] = ((item as { relations?: unknown[] }).relations ?? []).map(
      (raw) => {
        const rel = raw as { rel?: string; url?: string; attributes?: { name?: string } };
